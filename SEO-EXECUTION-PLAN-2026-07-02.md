...
- **W4.1 — money-page content rebuild** (`70342a6`): new `ServiceContent` component + a `content` slot on `PortfolioGallery`. All 4 service pages went from 150–300-word galleries to ~2,700–3,200 words of rendered HTML (several hundred words of unique prose each): intro, "what to expect", real local venues, a 5-Q visible FAQ with matching **FAQPage JSON-LD**, testimonial, and a booking CTA + phone. Maternity page leans into Copper Canyon/Argyle. Keyword-stuffed gallery alts replaced with honest ones. **Testimonials are real public Google reviews (first name + last initial) — Carly to confirm she's happy featuring them by name.**
- **W4.4 + W4.5 — new city pages + de-boilerplate** (`1392ed0`): added **Copper Canyon** (the sleeper — already ranks ~6.2 for "maternity photographer copper canyon" with no page), **Lewisville**, and **Lantana** via the existing template (real vibe/venues/FAQs, wired to sitemap + footer). Fixed the false alt text on location cards (generic images were labeled as specific parks) → honest "Family photography by Carly Gage in {city}".
...
- **W3 (highest local-traffic leverage) — all dashboard work, see `CGWeb/W3-DASHBOARD-CHECKLIST.md`.** GBP service-area redraw + category + photos + reviews engine; Yelp address hide; NAP standardization. None of this is in the repo. **This is now the biggest remaining lever.**
- **Confirm the testimonials.** The 4 money pages feature real public Google reviews (Tiffany T., Allison V., Hilary K.) by first name + last initial. Carly should confirm she's happy featuring them, or swap in her own preferred quotes (edit the `testimonial` object in each page).
- **Real per-city / per-service photos.** The new city pages and the location cards still use generic bento images (now with honest alt text). Swapping in real per-city photos where Carly has them (W4.4) further reduces template similarity — a content/asset task, not code.
- **W4.3 decision:** only `/portfolio/dallas-family-session` has a true duplicate (`/locations/dallas-family-photographer`). Recommend re-scoping just that one to a gallery title; leave the other 3 portfolio pages (they're sole owners + earn clicks). Not executed — needs your call.
- **W5 (lowest priority, deferred):** repoint money/location pages off multi-MB source JPGs to the existing `/images/optimized/*.webp` (only bento-1..5 have webp twins, so it's a partial swap; Vercel resizes on the fly anyway); trim `priority` image count; add a binary `favicon.ico` + PNG apple-touch icon (needs image tooling — the SVG favicon already works in modern browsers).
- **Homepage H1 (W2.2) — intentionally not shipped.** `v3/Hero.tsx` has the keyword in an `<h1>` already (two 10px eyebrows, mobile+desktop), with "Real Moments. Beautifully Kept." as the H2. Making the visible headline the H1 is a brand/design decision for Carly — low SEO impact since the keyword is already in an H1, and multiple H1s are fine for Google.
- **Nav "Sessions" dropdown — intentionally not shipped.** Money pages are now 1 click from every page via the footer, so this is a UX nicety, not a crawl fix, and the dropdown is a design/interaction change for Carly to approve.
